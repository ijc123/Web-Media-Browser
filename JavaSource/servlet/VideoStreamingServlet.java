...
import org.jboss.remoting.transport.coyote.CoyoteOutputStream;

import utils.FileUtils;
import video.Transcode2;
import video.TransferStream;
 ...
 */
public class VideoStreamingServlet extends LoadDataSegmentServlet {
            ...
            throws IOException
    {
        ...
        String fileName = FileUtils.splitPath(path)[2] + "." + type;
        ...
        try {
            ...
            transcode = new Transcode2(path, output);
    ...
    protected void copy(RandomAccessFile input, OutputStream output, long start, long length)
        throws IOException
    {
    	System.out.println(Long.toString(start) + " - " + Long.toString(length));
...
public class VideoStreamingServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 1L;
	private int DEFAULT_BUFFER_SIZE = 1048576;
			...
			throws ServletException, IOException 
	{
		...
		//String path = request.getParameter("path");
		String path = "G:/Inetpub/ftproot/hugo.mp4";
		
		String contentType = getServletContext().getMimeType(path);
			
		generateBinaryResponse(response, path, contentType);
		
	}
	
	private void generateBinaryResponse(HttpServletResponse response, String path, String mimeType) {
		...
		try {
			// Open streams.
			input = new BufferedInputStream(new FileInputStream(video), DEFAULT_BUFFER_SIZE);
