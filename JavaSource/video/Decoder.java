...
*/		
		...
		if(success < 0) {
			...
			throw new RuntimeException("XUGGLER DECODER: could not open input: " + input.getLocation().getDecodedURL()); 
		}
			...
			if(packet.getStreamIndex() == videoStreamIndex)
			{
				...
				while(offset < packet.getSize())
				{
					...
					if(bytesDecoded < 0) {
						...
						throw new RuntimeException("XUGGLER: error decoding video in: " + input.getLocation().getDecodedURL());
					}
